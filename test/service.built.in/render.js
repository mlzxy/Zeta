...
describe('$render',function(){
    describe('$render.renderHTML',function(){
        ...
        it('should render html successfully',function(done){
            demo.h('h0',function($scope,$render){
                $scope.res.end($render.renderHTML('<p>welcome</p>'));
            });
            request(demo.server(true)).
                get('/').
                expect(200).
        ...
        it('should support variable in html',function(done){
            demo.h('h0',function($scope,$render){
                $scope.res.end($render.renderHTML('<p>{{log}}</p>',{
                    log:'welcome'
                }));
            });
            request(demo.server(true)).
        ...
        it('should support swig string',function(done){
            demo.h('h0',function($scope,$render){
                $scope.res.end($render.renderHTML(
                    '{% for item in seq -%}'+'\n'
                    +'{{item}}'+'\n'
                    +'{%- endfor %}',{
                        seq:['1','2']
